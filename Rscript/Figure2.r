...
infil<-read.table("infiltration_estimation_for_tcga.csv",header = TRUE, sep=",",row.names = 1)
infil_CI<-dplyr::select(infil,ends_with("CIBERSORT"))
infil_CI$SampleID<-row.names(infil_CI)
NSUN1.infil<-merge(infil_CI,NSUN1.level,by="SampleID")%>% 
  pivot_longer(cols=matches("CIBERSORT"), names_to = "Celltype",values_to = "Fraction")
NSUN1.infil$Celltype<-gsub("_CIBERSORT", "",NSUN1.infil$Celltype)
NSUN1.infil$Celltype<-gsub("Myeloid.dendritic.cell.resting","Dendritic.cell.resting",NSUN1.infil$Celltype)
NSUN1.infil$Celltype<-gsub("Myeloid.dendritic.cell.activated","Dendritic.cell.activated",NSUN1.infil$Celltype)

########柱状图
NSUN1.infil.draw<- ggplot(data=NSUN1.infil,aes(x=Celltype,y=Fraction,fill=NSUN1.level))+ geom_boxplot()+
     theme_bw()+
 stat_summary(mapping=aes(x=Celltype,y=Fraction,color=NSUN1.level),fun="mean",geom="point",shape=21,size=3, fill="red",position=position_dodge(width=0.8))+
...
NSUN1.infil.draw
NSUN1.cor<-merge(infil_CI,NSUN1.level,by="SampleID")
NSUN1.cor.coef<-corr(NSUN1.cor[,2:(ncol(NSUN1.cor)-4)],NSUN1.cor$NSUN1)
NSUN1.cor.coef$Risk.score="NSUN1"
NSUN1.cor.coef$coefficient=as.numeric(NSUN1.cor.coef$coefficient)
NSUN1.cor.coef$Immune_cell<-gsub("_CIBERSORT", "",NSUN1.cor.coef$Immune_cell)
NSUN1.cor.coef$Immune_cell<-gsub("Myeloid.dendritic.cell.resting","Dendritic.cell.resting",NSUN1.cor.coef$Immune_cell)
NSUN1.cor.coef$Immune_cell<-gsub("Myeloid.dendritic.cell.activated","Dendritic.cell.activated",NSUN1.cor.coef$Immune_cell)
#######
